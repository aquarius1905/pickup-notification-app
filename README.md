...
- **当日キャンセルも事前キャンセルと同様の運用にする**: 現状、当日キャンセルはLINEメッセージのキーワード一致（`worker/src/index.ts`の`isCancelMessage`）で検出しているため、「キャンセルしないでください」のような打消し文でも誤検知しうる。事前キャンセルと同じくリッチメニュー・LIFFフォームからの操作に統一することを検討する。
- **LINE通知の既読確認**: 送迎通知をご家族が確認したかどうか（既読/未読）を施設側の画面で把握できるようにする。
- **利用者（ご家族）側でのキャンセル取り消し**: 一度送信したキャンセルを、ご家族側で取り消せるようにする。
