...
- **`families`テーブルのマイグレーション履歴の欠落**: `supabase/migrations`に`families`テーブルを新規作成するマイグレーションが存在せず、`notify_minutes`カラムの追加も含めて本番DBに手動で行われた変更がマイグレーション履歴に残っていない。このため`supabase start`でローカル環境をゼロから再現できない。`CREATE TABLE families`と欠けているカラム追加を補うマイグレーションを追加し、本番スキーマとGitの履歴を一致させる。
