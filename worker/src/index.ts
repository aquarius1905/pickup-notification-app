...
export default {
  async fetch(request, env): Promise<Response> {
    ...
    try {
      ...
      if (action === "listCancellations")
        return handleListCancellations(facilityId, env, supabaseHeaders);
      if (action === "listLogs")
        return handleListLogs(body, facilityId, env, supabaseHeaders);
      if (action === "notify")
...
async function authenticateFacility(
  apiKey: string,
  env: Env,
  headers: SupabaseHeaders,
...
async function handleResolveFacilityCode(
  ...
  const windowStart = new Date(
    Date.now() - FACILITY_CODE_RATE_WINDOW_MINUTES * 60 * 1000,
  ).toISOString();
  ...
  const tooManyAttempts =
    ...
    recentAttempts.length >= FACILITY_CODE_RATE_LIMIT;
...
type SupabaseResult<T> =
  { err: Response; rows?: never } | { err?: never; rows: T[] };

async function checkSupabaseResult<T>(
  res: Response,
...
async function handleList(
  ...
  const rows = (await res.json()) as (FamilyRecord & {
    cancellations: { date: string }[];
  })[];
  const todayPhases = await fetchTodayPhases(
    env,
    headers,
    rows.map((row) => row.id),
  );
...
async function handleListCancellations(
  facilityId: string,
  env: Env,
  headers: SupabaseHeaders,
): Promise<Response> {
...
async function handleListLogs(
  ...
  const search =
    typeof body.search === "string"
      ? body.search.trim().replace(/[*,]/g, "")
      : "";
  const cutoff = getPeriodCutoff(body.period);
  const eventType =
    body.eventType === "pickup_approaching" ||
    body.eventType === "dropoff_approaching"
      ? body.eventType
      : null;
...
async function handleNotify(
  ...
  const isApproaching =
    notifyType === "pickup_approaching" || notifyType === "dropoff_approaching";
  const isCompleted =
    notifyType === "pickup_completed" || notifyType === "dropoff_completed";

  if (!isApproaching && !isCompleted) {
    return jsonResponse(
      { ok: false, error: "通知の処理でエラーが発生しました。" },
      400,
    );
  }
...
async function handleUpdate(
  ...
  const { err, rows } = await checkSupabaseResult<FamilyRecord>(
    res,
    "利用者が見つかりません",
  );
  if (err) return err;
...
async function handleDelete(
  ...
  const { err } = await checkSupabaseResult<FamilyRecord>(
    res,
    "利用者が見つかりません",
  );
  if (err) return err;
...
async function handleUpdateFacility(
  ...
  const { err, rows } = await checkSupabaseResult<Facility>(
    res,
    "施設が見つかりません",
  );
  if (err) return err;
